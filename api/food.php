...
$app->get('/foods/name/{name}', function (Request $request, Response $response, $args) {
    $conn =$GLOBALS['connect'];
    $sql = 'select food.fid ,food.price,food.name,food.image from food where food.name like ?';
    $stmt = $conn->prepare($sql);
    $name = '%'.$args['name'].'%';
    $stmt->bind_param('s', $name);
    $stmt->execute();
